...
def calculate_cdf(data_array):
    ...
    sorted_data = np.sort(data_array.values)
    # Ensuring unique values by adding a small random noise;
    # adjust the scale as needed for data sensitivity
    unique_sorted_data = sorted_data + np.random.rand(len(sorted_data)) * 1e-10  
    cdf = np.arange(1, len(unique_sorted_data) + 1) / len(unique_sorted_data)
...
def calculate_monthly_cdfs(data_array):
    ...
    return data_array.groupby('time.month').apply(lambda x: calculate_cdf(x))
...
def calculate_tmy(
    ...
    fs_scores = monthly_cdfs.groupby('time.month').map(fs_function)
