...
def get_diffuse_horizontal_irradiance_series(
    ...
    """Estimate the clear-sky diffuse sky-reflected horizontal irradiance
    component over a location for a period in time.
    """
    ...
    if not quiet:
        if verbose > 0:
            ...
            print_irradiance_table_2(
                title=(
                    diffuse_horizontal_irradiance_series.title
    ...
    if statistics:
        ...
        print_series_statistics(
            ...
            timestamps=timestamps,
            groupby=groupby,
            title=diffuse_horizontal_irradiance_series.title + f"{IRRADIANCE_UNIT}",
            rounding_places=rounding_places,
        )
    if uniplot:
