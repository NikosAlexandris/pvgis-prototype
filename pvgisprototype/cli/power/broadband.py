...
def photovoltaic_power_output_series(
    ...
    apply_reflectivity_factor: Annotated[Optional[bool], typer_option_apply_angular_loss_factor] = ANGULAR_LOSS_FACTOR_FLAG_DEFAULT,
...
def photovoltaic_power_output_series_from_multiple_surfaces(
    ...
    surface_tilt: Annotated[Optional[list], typer_option_surface_tilt_multi] = [float(SURFACE_TILT_DEFAULT)],
    timestamps: Annotated[DatetimeIndex, typer_argument_timestamps] = str(now_utc_datetimezone()),
    ...
    if len(surface_tilt) != len(surface_orientation):
        ...
        logger.error(f"{exclamation_mark} Aborting as length of --surface_orientation and --surface_tilt is not the same!",
