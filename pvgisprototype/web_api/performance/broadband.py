...
async def get_photovoltaic_performance_analysis(
    ...
    """Analyse the photovoltaic performance for a solar surface of a specific
    technology, at a specific location and a given period.

    ### Input time series

    This function consumes internally :

    - solar irradiance time series from SARAH3 climate records (produced by the German Weather Service)
    - meteorological variables (temperature and wind speed) from ERA5 Reanalysis collection
    - spectral effect factor time series generated by .. for the reference year 2013

    ### Features

    - Get from simple to detailed output in form of **JSON**, **CSV**
