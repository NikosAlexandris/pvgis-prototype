...
async def get_calculate_noaa_timeseries_solar_position(
    ...
    solar_position_series = calculate_noaa_timeseries_solar_position(
        longitude=longitude,
        latitude=latitude,
    ...
    for quantity_name, quantity_object in solar_position_series.items():
        if not isinstance(quantity_object, list):
            try:
                ...
                solar_position_series[quantity_name] = [round(value, rounding_places) for value in angle_value_list]
            except AttributeError:
                time_value =  getattr(quantity_object, time_output_units)
                time_value_list = time_value.tolist()
                solar_position_series[quantity_name] = [round(value, rounding_places) for value in time_value_list]

    return {"solar_position_series": solar_position_series}
...
async def overview_series(
    ...
    angle_output_units: str = Query(RADIANS),
    rounding_places: Optional[int] = Query(5),
    group_models: Optional[bool] = Query(False, description='Visually cluster time series results per model'),
