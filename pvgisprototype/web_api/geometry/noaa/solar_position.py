...
async def get_calculate_noaa_timeseries_solar_position(
    ...
    if start_datetime is None and end_datetime is None:
        timestamps = [now_utc_datetimezone()]
        ...
        timestamps = [end_datetime]
    elif start_datetime is not None and end_datetime is not None:
        half_hour_intervals = int((end_datetime - start_datetime).total_seconds() / 1800)
        timestamps = [start_datetime + timedelta(minutes=30) * i for i in range(half_hour_intervals + 1)]
