...
def select_typical_month_iso_15927_4(
    ...
    wind_speed = wind_speed_series.value if hasattr(wind_speed_series, 'value') else wind_speed_series
    ...
    for month in range(1, 13):
        ...
        if verbose > 2:
            print(f"  Long-term wind speed mean for month {month}: {long_term_wind_speed_mean:.3f}")
        ...
        for year in candidate_years:
            ...
            wind_speed_deviation = abs(this_year_month_wind_speed_mean - long_term_wind_speed_mean)
            wind_speed_deviations.append(wind_speed_deviation)
            
            if verbose > 2:
                print(f"    Year {year} : mean = {this_year_month_wind_speed_mean :.3f}, deviation = {wind_speed_deviation:.3f}")
        ...
        if verbose > 1:
            print(f"✓ Month - Year {month} - {selected_year} "
