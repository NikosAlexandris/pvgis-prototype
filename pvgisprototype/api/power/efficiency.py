...
def adjust_temperature_series(
    ...
    if temperature_model.value  == ModuleTemperatureAlgorithm.faiman:
        if wind_speed_series is not None:
            ...
            if len(photovoltaic_module_efficiency_coefficients) < expected_number_of_coefficients:
...
def calculate_efficiency_factor_series(
    ...
    relative_irradiance_series = 0.001 * irradiance_series  # Conversion to Unit PV Power ?
    ...
    components_container = {
        ...
        'even_more_extended': lambda: {
            ...
            LOW_IRRADIANCE_COLUMN_NAME: relative_irradiance_series <= radiation_cutoff_threshold,
...
def calculate_spectrally_corrected_effective_irradiance(
    ...
    components_container = {
        ...
        'extended': lambda: {
            ...
            SPECTRAL_EFFECT_COLUMN_NAME: spectral_effect_series if (verbose > 1 and spectral_effect_series.size > 0) else NOT_AVAILABLE,
            SPECTRAL_EFFECT_PERCENTAGE_COLUMN_NAME: spectral_effect_percentage_series if (verbose > 1 and spectral_effect_percentage_series.size > 0) else NOT_AVAILABLE,
        } if verbose > 1 else {},
    }
...
def calculate_pv_efficiency_series(
    ...
    components_container = {
        ...
        'more_extended': lambda: {
            TITLE_KEY_NAME: EFFICIENCY_NAME + ' & components',
            TEMPERATURE_DEVIATION_COLUMN_NAME: efficiency_series.components[TEMPERATURE_DEVIATION_COLUMN_NAME],
        } if verbose > 2 else {},
