...
def create_panel_for_table(table):
    return Panel(table, expand=True, padding=(1, 2))
...
def create_symbols_table():
    ...
    for category, symbols in SYMBOL_GROUPS_DESCRIPTIONS.items():
        ...
        category_table.add_column(
            "Symbol", justify="right", style="bright_blue", no_wrap=True
        )
...
def layout_panels_with_categories(symbol_group_descriptions):
    ...
    columns = Columns(panels, expand=True, equal=False, padding=3)
    return columns
...
print(
    console.export_html(
        inline_styles=True, code_format="<pre><code>{code}</code></pre>"
    )
)
