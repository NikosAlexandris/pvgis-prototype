...
from get_solar_data_usno_api import query_usno

data = pd.read_excel("../data/algorithms_NOAA.xlsx")

# Testing Event Time
cases = "Solar Azimuth: NOAA vs USNO"
print(f"-> Generating cases for module: {cases}")


with open("../algorithms/noaa/cases/cases_solar_azimuth_usno.py", "w") as file:
    ...
    for _, row in data.iterrows():
        ...
        latitude_degrees = float(row["LATITUDE [DEGREES]"])
        longitude_degrees = float(row["LONGITUDE [DEGREES]"])

        date = datetime.strptime(f"{year}-{month}-{day} {hour}:{minutes}:{seconds}", "%Y-%m-%d %H:%M:%S")
        target, _, _, _ = query_usno(longitude=longitude_degrees, latitude=latitude_degrees, date=date)
        ...
        if target is not None:
            ...
            file.write(f"        \"dtype\": DATA_TYPE_DEFAULT,\n")
            file.write(f"        \"array_backend\": 'ARRAY_BACKEND_DEFAULT',\n")
    ...
    for (_, row), target in zip(data.iterrows(), target_list):
        if target is not None:
            year = int(row["YEAR"])
            month = int(row["MONTH"])
            day = int(row["DAY"])
